class ngram {
	...
	; --- Static Methods ---
	generate(param_array, param_groupsize:=1, param_separator:="") {
		if (!isObject(param_array)) {
			param_array := strSplit(param_array, param_separator)
		}
		ngrams := []
		if (isObject(param_groupsize)) {
			for key, value in param_groupsize {
				ngrams.push(this.generate(param_array, value, param_separator))
			}
		} else {
			loop, % param_array.count() {
				thisIteration := this._slice(param_array, A_Index, A_Index + param_groupsize - 1)
				if (thisIteration.count() == param_groupsize) {
